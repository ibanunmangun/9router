...
it("uses the shared Claude Code version in the billing header", () => {
  const body = applyCloaking({ messages: [] }, "sk-ant-oat-test", "session-id");
  expect(body.system[0].text).toMatch(new RegExp(`^x-anthropic-billing-header: cc_version=${CLAUDE_CLI_VERSION.replace(/\./g, "\\.")}\\.`));
});
